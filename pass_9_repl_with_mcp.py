# repl.py
"""A chat program that gets its tools from an external MCP server.

Instead of writing tool functions in this file, we connect to a
running tool server (here: open-meteo-mcp for weather) and let the
model use whatever tools it offers. Long-term memory is preserved.

Requires Redis:
    docker run -d -p 6379:6379 --name redis redis/redis-stack-server:7.4.0-v8
and the open-meteo-mcp server:
...
# Silence an unrelated deprecation warning from the MCP client library.
...
# Same anti-hallucination prompt as pass 8: tell the model to admit
# when a tool failed instead of making up data.
NO_FABRICATION = (
...
async def main() -> None:
    ...
    print(
        f"Chatting with {OLLAMA_MODEL} (with long-term memory and MCP tools)."
    )
    print(
        f"MCP tools available: {', '.join(t.name for t in tools)}"
    )
    print(
